...
import { MagnifyingGlassIcon, Bars3Icon, XMarkIcon, ShoppingBagIcon, UserCircleIcon } from '@heroicons/react/24/outline';
...
export default function Header() {
  const { status } = useSession();                       // <- runs in component
  const isAuthed = status === 'authenticated';
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const navItems = useMemo(
    () => (isAuthed ? [...BASE_LINKS, { name: 'Profile', href: '/profile' }] : BASE_LINKS),
    [isAuthed]
  );

  return (
    <header className="bg-[#2D3748] shadow-sm sticky top-0 z-50" data-header="site-header-v2">
      <nav className="mx-auto max-w-[1280px] px-[5px]">
        <div className="flex justify-between items-center py-4 md:py-6">
          ...
          <div className="hidden md:flex items-center ml-[2rem] space-x-[2rem]">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-white hover:text-gray-300 transition-colors duration-200">
                {item.name}
              </Link>
            ))}
          </div>
      ...
      {/* Mobile slide-in */}
      <div className={`fixed inset-0 z-40 transform transition-transform duration-300 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="absolute inset-0 bg-[#2D3748] shadow-xl">
          <div className="flex justify-end p-4">
            <button onClick={() => setIsMobileMenuOpen(false)} className="text-white hover:text-gray-300 focus:outline-none">
