...
import { ShoppingCart, Truck, CreditCard, type LucideIcon } from 'lucide-react';
...
// ---------- Utils ----------
const calculateSummary = (items: CartItem[]): SummaryData => {
...
const CartItemRow: React.FC<CartItemRowProps> = ({ item, onUpdateQuantity }) => (
  <div className="flex py-6 border-b border-gray-200">
    ...
    <div className="flex-grow px-4 md:px-8">
      ...
      <p className="font-medium text-gray-900 mt-2 text-xl">${item.unitPrice.toFixed(2)}</p>
    </div>

    {/* Qty */}
    <div className="flex items-center justify-end">
      <QuantityControl quantity={item.quantity} onChange={(q) => onUpdateQuantity(item.id, q)} />
    </div>
  </div>
);
...
const CheckoutPage: React.FC = () => {
  ...
  const tabs: { id: Tab; name: string; icon: LucideIcon }[] = [
        ...
        {activeTab === 'shipping' && (
          <div className="py-20 text-center">
            ...
            <div className="mt-8 flex justify-center space-x-4">
              <button onClick={() => setActiveTab('cart')} className="bg-gray-200 text-gray-700 py-2 px-6 rounded-sm">
                Back
              </button>
              <button onClick={handleNext} className="bg-amber-800 text-white py-2 px-6 rounded-sm">
                Next
              </button>
            </div>
          </div>
